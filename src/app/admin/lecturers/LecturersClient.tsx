    ...
    <div ref={container} className="max-w-7xl mx-auto space-y-8 font-sans pb-10">
      <div className="flex justify-between items-end anim-item">
        <div>
          ...
          <p className="text-[#5D6065] text-lg mt-2 font-medium">Provision Lecturer credentials hierarchically.</p>
        </div>
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-[#E4D4CC] anim-item flex flex-col md:flex-row gap-4 items-end">
        ...
        <button onClick={handleFilter} disabled={!filterDeptId || isLoading} className="px-8 py-3.5 bg-[#4A3131] text-white font-bold rounded-xl hover:bg-[#5a3f3f] transition active:scale-[0.98] disabled:opacity-70 flex items-center gap-2">
        ...
        <div className="xl:col-span-1 bg-white p-8 rounded-3xl shadow-sm border border-[#E4D4CC] anim-item h-max">
          ...
          <form className="space-y-4" onSubmit={handleCreate}>
            <div>
              ...
              <input name="name" required placeholder="e.g. Dr. Jane Doe" className="w-full p-3 border-2 border-[#E4D4CC] rounded-xl focus:border-[#4A3131] outline-none text-[#4A3131] font-medium" />
            </div>
            ...
            {lecturers.length === 0 ? (
              ...
              <table className="w-full text-left text-sm text-[#5D6065]">
                ...
                <tbody className="divide-y divide-[#E4D4CC]">
                  {lecturers.map(l => (
                    <tr key={l.id} className="hover:bg-[#F4EFEA]/20 transition group">
      ...
      {showCSVModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="bg-white p-8 rounded-3xl w-full max-w-md shadow-2xl">
             ...
             <p className="text-[#5D6065] mb-6 font-medium text-sm">Target Department is required before uploading (Name, Email, Staff ID).</p>
             ...
             {csvDeptId ? (
               ...
               <div className="border-2 border-dashed border-[#E4D4CC] rounded-2xl h-40 flex items-center justify-center flex-col bg-[#F4EFEA]/30 mb-6 opacity-50">
                 <span className="font-bold text-[#5D6065] text-center px-4">Select Target Department to reveal upload</span>
               </div>
             )}
