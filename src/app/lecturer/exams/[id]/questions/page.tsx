    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 pb-16">
            <section className="bg-white border border-accent rounded-3xl p-7 shadow-sm">
                <p className="text-xs font-bold tracking-[0.18em] uppercase text-secondary">Question Builder</p>
                <h1 className="mt-2 text-3xl font-black text-primary">{exam.title}</h1>
                <p className="mt-1 text-sm font-semibold text-secondary">{exam.course.code}</p>
                <div className="mt-5 flex items-center gap-3">
                    <Link
                        href="/lecturer/exams"
                        className="px-4 py-2 rounded-lg border border-accent text-sm font-bold text-primary hover:bg-accent"
                    >
                        Back to Exams
                    </Link>
            ...
            <section className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                <form action={manualAction} className="bg-white border border-accent rounded-3xl p-7 space-y-4 shadow-sm">
                    ...
                    <button
                        type="submit"
                        className="w-full h-12 rounded-xl bg-primary text-white font-bold hover:bg-primary/90"
                    >
                        Add Question
                    </button>
                </form>
                ...
                <form action={csvAction} className="bg-white border border-accent rounded-3xl p-7 space-y-4 shadow-sm">
                    ...
                    <input
                        ...
                        accept=".csv,text/csv"
                        required
                        className="w-full p-3 rounded-xl border-2 border-dashed border-accent bg-accent"
                    />
                    <button
                        type="submit"
                        className="w-full h-12 rounded-xl bg-primary text-white font-bold hover:bg-primary/90"
            ...
            <section className="bg-white border border-accent rounded-3xl shadow-sm overflow-hidden">
                ...
                {questions.length === 0 ? (
                    <p className="p-6 text-sm font-semibold text-secondary">No questions added yet.</p>
                ) : (
                    <ol className="divide-y divide-accent">
                        {questions.map((question) => (
                            <li key={question.id} className="p-5">
                                ...
                                <p className="mt-2 text-xs font-semibold text-secondary/80">Correct: {question.correctAnswer}</p>
                            </li>
                        ))}
                    </ol>
                )}
